...
export class Server extends EventEmitter {
    ...
    async onRequest(request, response) {
        try {
            ...
            if (handlerOpts.customResponseFunction) {
                ...
                await handleCustomResponse(request, response, handlerOpts);
                return;
            }
            if (handlerOpts.upstreamProxyUrlParsed && SOCKS_PROTOCOLS.includes(handlerOpts.upstreamProxyUrlParsed.protocol)) {
                ...
                await forwardSocks(request, response, handlerOpts);
                return;
            }
            this.log(proxyChainId, 'Using forward()');
            await forward(request, response, handlerOpts);
        }
        catch (error) {
     ...
     */
    async close(closeConnections = false) {
        if (closeConnections) {
